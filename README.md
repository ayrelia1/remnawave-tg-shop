...
-   **Курсы валют:** Раздел «Системные функции → Курсы валют». Каждый платёж пересчитывается в базовую валюту в момент оплаты, и курс сохраняется на самом платеже — изменение курса не переписывает историю, а применяется к новым платежам. Платежи в валюте без курса не попадают в отчёты, и админка о них предупреждает.
...
2.  **Создайте и настройте файл `.env`:**
    ...
    | `PARTNER_PAYOUT_CURRENCY` | Базовая валюта: в неё пересчитывается каждый платёж и в ней показывается баланс партнёра. | `RUB` |
