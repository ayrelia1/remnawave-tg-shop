...
-   **Бонус за имя в Telegram:** При `NAME_BONUS_ENABLED=True` после любой успешной покупки можно получать 7 дней доступа раз в 30 дней через «Задания» → «7 дней за имя Telegram». В поле «Имя» должен быть `@MansurVPN_BOT` или `Mansur VPN` в любой части имени, строго в указанном регистре. Бот проверяет имя каждые 30 минут в течение 7 дней; если в имени не останется ни одного варианта, списывает оставшуюся часть бонуса. Если подписка истекла, бонус создаёт новый доступ на 7 дней.
