...
-   **Бонус за имя в Telegram:** При `NAME_BONUS_ENABLED=True` после любой успешной покупки можно получать 5 дней доступа раз в 30 дней через «Задания» → «5 дней за имя Telegram». В поле «Имя» должно быть `Mansur VPN` или `MansurVPN` в любой части имени и любом регистре. Бот проверяет имя каждые 30 минут в течение 5 дней; при удалении упоминания списывает оставшуюся часть бонуса. Если подписка истекла, бонус создаёт новый доступ на 5 дней.
